...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        "self.visited_corners = [self.startingPosition]"
    def getStartState(self):
...
def cornersHeuristic(state, problem):
    ...
    current = state[0]
    cost = 0
    while not corners_to_visit == []:
        pqueue = util.PriorityQueue()
        for corner in corners_to_visit:
            "m_dist = ((current[0] - corner[0])**2 + (current[1] - corner[1])**2)**0.5"
            m_dist = util.manhattanDistance(current, corner)
            pqueue.push((corner, m_dist), m_dist)
        nearest_state = pqueue.pop()
        current = nearest_state[0]
        cost = cost +  nearest_state[1]
        corners_to_visit.remove(nearest_state[0])
    return cost
...
def foodHeuristic(state, problem):
    ...
    for food in food_list:
        m_dist = util.manhattanDistance(position, food)
        
        e_dist = ((position[0] - food[0])**2 + (position[1] - food[1])**2)**0.5
        '''
        if e_dist > (2)**0.5:
            m_dist = m_dist*0.9
        '''
