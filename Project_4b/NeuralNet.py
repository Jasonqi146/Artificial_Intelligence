...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        """YOUR CODE"""
        new_inActs = [1]
        new_inActs.extend(inActs)
        for i in range(len(new_inActs)):
            updates = alpha * delta * new_inActs[i]
            self.weights[i] += updates
            totalModification += abs(updates)
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        """YOUR CODE"""
        result_list = []
        result_list.append(list(inActs))
        for i in range(self.numLayers):
            output = []
            if i != self.numLayers -1:
                for perceptron in self.hiddenLayers[i]:
                    output.append(perceptron.sigmoidActivation(result_list[i]))
    ...
    def backPropLearning(self, examples, alpha):
        ...
        """
        #keep track of output        #keep track of output
        averageError = 0
        ...
        for example in examples:#for each example
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    delta = 0.0
                    for percNum in range(len(nextLayer)):
                        nextWeight = nextLayer[percNum].weights[neuronNum+1]
                        delta += nextWeight*deltas[0][percNum]
                    delta *= gPrime
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    """
        ...
        #else :
        #    print '.',
    weightMod += 1 + weightChangeThreshold
    ...
    for testExam in examplesTest:
        inputs = testExam[0]
        outputs = nnet.feedForward(inputs)[-1]
        entire_correct = True
        for i in range(len(outputs)):
            if round(outputs[i]) != testExam[1][i]:
                entire_correct = False
        if entire_correct == True:
            testCorrect += 1
        else:
            testError += 1
        
    testAccuracy = testCorrect * 1.0/(testError+testCorrect)
